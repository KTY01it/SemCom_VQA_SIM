...
from src.methods.neural_semantic_planner import load_nsp_model, predict_candidate
...
def parse_args():
    ...
    parser.add_argument("--method-name", default="nsp_v1")
...
def main():
    ...
    model = load_nsp_model(args.model)
    ...
    for sample_idx, row in enumerate(sg_rows):
        ...
        for rank_index, t in enumerate(triplets):
            select_prob, keep, _ = predict_candidate(
                model=model,
                triplet=t,
