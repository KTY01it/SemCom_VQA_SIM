...
from src.methods.neural_semantic_reranker import safe_shifted_id
...
def main():
    ...
    parser.add_argument("--out", default="results/nsp/nsp_v5_semantic_train.pt")
    ...
    for sample_pos, row in enumerate(sg_rows):
        qid = row["question_id"]
        ...
        valid_rows.append((sample_pos, sample, triplets))
    ...
    for group_id, (sample_pos, sample, triplets) in enumerate(valid_rows):
        q = sample.get("question", "")
        q_emb = q_cache[q]

        utilities = []
        for t in triplets:
            u = oracle_utility_score(
                triplet=t,
        ...
        # Select positives by oracle utility, not DBSS.
        ranked_idx = sorted(range(len(triplets)), key=lambda i: utilities[i], reverse=True)
        selected = set(i for i in ranked_idx[: args.n_top] if utilities[i] > 0.0)

        if not selected and ranked_idx:
            selected.add(ranked_idx[0])

        probs = softmax_teacher_probs(utilities, args.teacher_temperature)
        ...
        for i, t in enumerate(triplets):
            ...
            q_embs.append(q_emb)
            t_embs.append(t_emb)
            teacher_probs.append(probs[i])
            select_labels.append(1.0 if i in selected else 0.0)
            utility_scores.append(utilities[i])
            mask_labels.append([
            ...
            total_candidates += 1
            total_selected += int(i in selected)
    ...
    out = {
        ...
        "teacher_prob": torch.tensor(teacher_probs, dtype=torch.float32),
        "select_label": torch.tensor(select_labels, dtype=torch.float32),
        "utility_score": torch.tensor(utility_scores, dtype=torch.float32),
