...
def main():
    ...
    parser.add_argument("--teacher-tau", type=float, default=2.0)
    parser.add_argument("--out", default="results/nsp/nsp_v3_listwise_train.csv")
    ...
    for sample_pos, row in enumerate(sg_rows):
        ...
        rank_by_key = {}
        for r, t in enumerate(dbss_ranked[: args.n_top]):
            rank_by_key[triplet_key(t)] = r

        # Raw teacher weights before per-question normalization.
        raw_weights = []
        for t in triplets:
            k = triplet_key(t)
            if k in rank_by_key:
                # Larger weight for higher DBSS rank.
                w = math.exp(-float(rank_by_key[k]) / max(1e-6, args.teacher_tau))
            else:
                # Small non-zero mass for negatives.
                w = 1e-3
            raw_weights.append(w)

        s = sum(raw_weights)
        teacher_probs = [w / max(1e-12, s) for w in raw_weights]

        for candidate_index, t in enumerate(triplets):
            key = triplet_key(t)
            select_label = int(key in rank_by_key)
            ...
            base = {
                ...
                "teacher_rank": rank_by_key.get(key, -1),
                "teacher_prob": teacher_probs[candidate_index],
    ...
    if not rows:
        raise RuntimeError("No NSP-v3-listwise rows were produced.")
