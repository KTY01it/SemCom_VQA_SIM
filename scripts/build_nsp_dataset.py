...
from src.methods.nst_budget_labeling import field_importance, infer_budget_keep_label
from src.methods.nst_features import build_nst_v2_features, nst_v2_feature_names
from src.utils.config import load_yaml


def triplet_utility(triplet, question, keywords, answer, question_type):
    ...
    # Selection utility: positive field evidence + relation evidence.
    return max(imp.values()) + 0.25 * sum(max(0.0, v) for v in imp.values())
...
def main():
    ...
    parser.add_argument("--out", default="results/nsp/nsp_train.csv")
    ...
    for row in sg_rows:
        ...
        for idx, t in enumerate(triplets):
            ...
            scored.append((idx, t, u))

        # Select top utility candidates as pseudo positive triplets.
        scored_sorted = sorted(scored, key=lambda x: x[2], reverse=True)
        selected_indices = {
            idx for idx, _, u in scored_sorted[: args.n_top]
            if u > 0.0
        }

        # If no candidate has positive utility, keep the best one as weak supervision.
        if not selected_indices and scored_sorted:
            selected_indices.add(scored_sorted[0][0])

        for rank_index, t, u in scored:
            select_label = int(rank_index in selected_indices)
    ...
    if not rows:
        raise RuntimeError("No NSP rows were produced.")
